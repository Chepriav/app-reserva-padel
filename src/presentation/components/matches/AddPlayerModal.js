...
function getFilteredUsers(users, search, currentPlayers) {
  ...
  if (term) {
    filtered = users.filter(u =>
      ...
      u.apartment?.toLowerCase().includes(term)
    );
  }
  ...
  return filtered.slice(0, 20).map(u => ({
    ...u,
    alreadyAdded: players.some(p => p.type === 'urbanizacion' && p.user?.id === u.id),
  }));
}
  ...
  return (
    ...
    >
      <View style={styles.userAvatar}>
        {user.profilePhoto ? (
          <Image source={{ uri: user.profilePhoto }} style={styles.userAvatarImage} />
        ) : (
