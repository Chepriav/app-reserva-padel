...
export default function PlayersEditor({
  ...
  user,
  onAddPlayer,
  onAbrirModalPlayer = onAddPlayer,
  onRemovePlayer,
  isClass,
  isLesson = isClass,
  maxParticipants,
  ...
  return (
    <>
      <Text style={styles.label}>
        {isLesson ? 'Alumnos' : 'Jugadores'} ({totalPlayers}/{maxPlayers})
      </Text>
      <View style={styles.container}>
        ...
        {playersList.map((player, index) => (
          <PlayerRow
            ...
            level={player.level}
            isExternal={player.type === 'externo'}
        ...
        {playersList.length < maxAddable && (
          <TouchableOpacity style={styles.addButton} onPress={onAbrirModalPlayer}>
            <Text style={styles.addButtonText}>
              + Añadir {isLesson ? 'alumno' : 'jugador'}
            </Text>
          </TouchableOpacity>
        )}
      </View>
